...
def phase_main(ctx, W: Path, res: Path):
    ...
    dev = pd.concat([df1[df1.config_id.isin(sum(top.values(), []))], df2], ignore_index=True)
    sel, final_steps = {}, {}
    for kind in ARMS:
        d = dev[dev.model == kind]
        agg = d.groupby('config_id').best_val_mse_path.mean().sort_values()
        sel[kind] = agg.index[0]
        chosen = d[d.config_id == sel[kind]]
        scaled = []
...
def phase_ablation(ctx, W: Path, res: Path):
    lock = json.loads((W / 'selection_lock.json').read_text())
...
def main():
    ...
    ap.add_argument('--phase', required=True, choices=['main', 'ablation', 'controlled', 'package'])
    ...
    {'main': phase_main, 'ablation': phase_ablation,
