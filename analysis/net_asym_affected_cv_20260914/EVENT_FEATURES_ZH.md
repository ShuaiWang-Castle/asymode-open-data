...
- **论文阈值 Λ = 1（n 取训练事件数）方向对，位置只能大致确定（第 4、5 节）。**
  - NET 更好的起点 Λ̂ 明显更大：中位数 0.46 对 0.16，均值 0.83 对 0.36。
  - ASYM 更好的时刻在 1 以下；NET 更好的时刻大多也在 1 以下，不在 1 以上。
  - 起点级，切点取 1 分开两组的效果与最佳切点几乎相同，切点高于 1 明显变差；事件级各项估计在 0.3–0.7，略低于 1。
  - 按 Λ̂ > 1 选 NET 仍不能降低总误差。
...
**问题：** 按论文定义取 n = 该事件所在外层折的独立训练事件数（平均约 15 个），检验实际的模型优劣是否在 Λ̂ = 1 处翻转。
- 区间来自按时间重叠组做的重采样。
- 代码：`code/threshold_accuracy.py`；结果：`results/THRESHOLD_ACCURACY.json`。

| 估计 | 事件级（26 个事件） | 起点级（535 个起点） |
|---|---:|---:|
| 胜负翻转点：P(NET 更好) = ½ 时的 Λ̂ | 0.72（95% 区间 0.39–1.85） | 1.71（0.69–13.50） |
| 总误差最小的切换点 | 0.34（0.32–1.44） | 一律用 NET 最优（重采样中位 0.10） |
| 按 Λ̂ > 1 选 NET，误差相对一律用 NET | +0.7% | +5.5% |

起点级按 Λ̂ 分箱（事件等权）：

| Λ̂ 分箱中位数 | 起点数 | NET 更好的起点比例（事件等权） | 平均相对 Δ（正数偏 ASYM） |
|---:|---:|---:|---:|
| 0（信号不为正） | 137 | 22% | +28% |
| 0.03 | 50 | 17% | +32% |
| 0.10 | 50 | 24% | +22% |
...
![Λ̂ 与实际优劣](figures/threshold_accuracy.png)

- **作为胜负翻转点，Λ = 1 与数据一致。** 两个分辨率的区间都包含 1。起点级 NET 更好的比例在 Λ̂ 约 1.2 处过半；相对 Δ 在 Λ̂ 约 0.5–1.1 附近开始转负，Λ̂ 约 2 时明显偏 NET。
- **但定位精度很低。** 事件级区间横跨 0.39–1.85，起点级更宽；约四分之一的起点检测不到正信号。
- **按 Λ̂ = 1 做决策不能降低总误差。** 原因有两个：
  - 单个起点或事件的 Λ̂ 噪声很大。
  - 误判代价不对称：NET 更好的往往是停电大、误差大的情形，判错一次损失很大；ASYM 更好的多是小事件，赢得少。

  所以按总误差最优的切换点低于 1（事件级 0.34）。
- **停电水平阈值同样不精确：** 0.88%（95% 区间 0.59%–1.89%）。

## 5. NET 更好与 ASYM 更好的时刻：Λ̂ 的均值与中位数

**做法：** 按 PI 的建议（2026-09-15），把预测起点按实际更优的模型分成两组，比较两组 Λ̂ 的均值和中位数；事件级同样比较。如果阈值 1 准确，NET 更好的时刻应在 1 以上，ASYM 更好的应在 1 以下。
- Λ̂ 同第 4 节：n 取训练事件数；信号不为正（Ŝ ≤ 0）时记为 0。
- 起点的实际更优，按该起点受灾县路径 MSE 的 5 个 seed 均值判定。
- 敏感性：只保留信号为正的起点；只保留两模型差距至少为 NET MSE 5% 的起点，即去掉几乎打平的起点。
- 区间来自按重叠分量的重采样（2000 次）。
- 代码：`code/lambda_group_compare.py`；结果：`results/LAMBDA_GROUP_COMPARISON.json`。

**两组各自的 Λ̂：**

| 单位 | 实际更优 | 个数 | Λ̂ 中位数（95% 区间） | Λ̂ 均值（95% 区间） | Λ̂ > 1 的比例 | 信号不为正的比例 |
|---|---|---:|---:|---:|---:|---:|
| 全部起点 | NET | 163 | 0.46（0.27–0.67） | 0.83（0.60–1.04） | 33% | 18% |
|  | ASYM | 372 | 0.16（0.09–0.20） | 0.36（0.28–0.47） | 9% | 29% |
| 只含正信号的起点 | NET | 133 | 0.66（0.46–1.02） | 1.02（0.74–1.25） | 41% | 0% |
|  | ASYM | 265 | 0.31（0.23–0.37） | 0.51（0.41–0.61） | 12% | 0% |
| 两模型差距 ≥ 5% 的起点 | NET | 132 | 0.45（0.22–0.73） | 0.85（0.57–1.06） | 33% | 18% |
|  | ASYM | 352 | 0.14（0.08–0.20） | 0.35（0.27–0.44） | 8% | 29% |
| 事件 | NET | 10 | 0.85（0.65–1.65） | 1.06（0.71–1.53） | 30% | 0% |
|  | ASYM | 16 | 0.30（0.15–0.56） | 0.47（0.22–0.74） | 19% | 0% |

**两组之间：**

| 单位 | 中位数之差 | 均值之差 | P(NET 更好时刻的 Λ̂ 更大) | 分界点 | 分界点处的区分度 | Λ̂ = 1 处的区分度 |
|---|---:|---:|---:|---:|---:|---:|
| 全部起点 | 0.31（0.12–0.52） | 0.47（0.24–0.67） | 0.65（0.58–0.71） | 0.43（0.10–1.03） | 0.26 | 0.25（0.14–0.32） |
| 只含正信号的起点 | 0.35（0.14–0.74） | 0.51（0.23–0.74） | 0.68（0.60–0.74） | 0.96（0.36–1.09） | 0.30 | 0.29（0.16–0.40） |
| 两模型差距 ≥ 5% 的起点 | 0.30（0.08–0.61） | 0.50（0.23–0.71） | 0.66（0.57–0.73） | 0.96（0.09–1.05） | 0.26 | 0.24（0.12–0.34） |
| 事件 | 0.55（0.27–1.38） | 0.59（0.17–1.11） | 0.85（0.67–0.99） | 0.56（0.32–0.69） | 0.71 | 0.11（−0.22 至 0.49） |

- 括号内是 95% 区间。
- **P(NET 更好时刻的 Λ̂ 更大)：** 两组各随机取一个时刻，NET 更好的那个 Λ̂ 更大的概率；0.5 表示分不开。
- **区分度：** NET 更好的时刻中 Λ̂ 高于切点的比例，减去 ASYM 更好的时刻中高于切点的比例。1 表示完全分开，0 表示分不开。它不受两组时刻数多少的影响，等于下图左侧两条累积分布曲线在该切点处的竖直距离。
- **分界点：** 区分度最大的切点。

**区分度随切点的变化：**

| 单位 | 切点 0.1 | 0.2 | 0.3 | 0.5 | 0.7 | **1** | 1.5 | 2 | 3 |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 全部起点 | 0.20 | 0.20 | 0.22 | 0.23 | 0.23 | 0.25 | 0.16 | 0.09 | 0.03 |
| 只含正信号的起点 | 0.15 | 0.17 | 0.21 | 0.24 | 0.26 | 0.29 | 0.19 | 0.11 | 0.03 |
| 两模型差距 ≥ 5% 的起点 | 0.22 | 0.19 | 0.22 | 0.23 | 0.24 | 0.24 | 0.17 | 0.12 | 0.03 |
| 事件 | 0.06 | 0.38 | 0.44 | 0.65 | 0.51 | 0.11 | 0.14 | 0.10 | 0.00 |

![两组 Λ̂ 的分布](figures/lambda_group_comparison.png)

- **方向稳健：NET 更好的时刻 Λ̂ 明显更大。**
  - 起点级：中位数 0.46 对 0.16，均值 0.83 对 0.36，差值区间都不含 0。去掉无正信号或几乎打平的起点后不变。
  - 事件级：中位数 0.85 对 0.30，均值 1.06 对 0.47。
- **ASYM 更好的时刻在 1 以下，符合论文。** 起点级中位数 0.16（0.09–0.20），91% 的起点低于 1。
- **NET 更好的时刻并不在 1 以上，而是在 1 附近或更低。**
  - 起点级中位数 0.46，区间 0.27–0.67 不含 1；只有 33% 的起点高于 1。均值 0.83 被少数大值拉高。
  - 只看正信号的起点，中位数 0.66（0.46–1.02），均值 1.02。
  - 事件级中位数 0.85、均值 1.06，区间都包含 1。
- **起点级，1 是两组分界的上沿。** 切点在 0.1–1 之间，区分度都在 0.20–0.26；1 处的 0.25 与最佳切点（0.43 处 0.26）几乎相同。只看正信号的起点，最佳切点就是 0.96。切点高于 1 后区分度迅速下降（1.5 处 0.16，2 处 0.09）。
- **事件级最佳切点约 0.56（0.32–0.69），1 偏高。** 1 处区分度只有 0.11，区间包含 0。但这个切点是在同一批 26 个事件上选出来的，并且取决于少数 Λ̂ 落在 0.8–0.9 的 NET 更好事件，很不稳定。
- **均值和中位数能确定方向，不能单独确定阈值位置。** ASYM 在约 70% 的起点更好，很多起点两模型接近打平，实际胜负本身带噪声。即使真实分界正好在 1，NET 更好的一组里也会混入不少 Λ < 1、只是这次 NET 碰巧更好的起点，把这一组的中位数拉到 1 以下。判断位置应看区分度。
- **与第 4 节合起来：**
  - 分界在 1 或略低于 1 都与数据相容；切点高于 1 时，区分度和总误差都更差。
  - 第 4 节起点级翻转点 1.71 偏高，是因为翻转点还取决于两组的多少，而 ASYM 更好的起点占多数。
  - 事件级的几个估计（翻转点 0.72、总误差最优 0.34、最佳切点 0.56）都略低于 1，但只有 26 个事件，区间都很宽。
