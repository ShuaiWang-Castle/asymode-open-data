...
Novelty cannot consist of “physics-informed,” a neural ODE, a cumulative weather state, rain–wind interactions, hourly forecasts, geography, or fragility curves alone. Rackauckas et al.'s [universal differential equations](https://arxiv.org/html/2001.04385) establish the general learned-closure approach, and Kidger et al.'s [Neural CDE](https://proceedings.neurips.cc/paper/2020/hash/4a5876b450b45371f6cfe5047ac8cd45-Abstract.html) already provides a general, differentiable state driven by an observed multivariate path. Zhu et al.'s [grid-resilience model](https://arxiv.org/html/2109.09711v3) learns discounted histories of 34 weather variables, nonlinear cross-weather effects, geographic vulnerability, and decaying outage influence. [Chen et al.'s GDF-NODE v3](https://arxiv.org/html/2502.18321) already conserves unaffected, outaged, and restored customer stocks while neural weather/socioeconomic rates drive the dynamics. [Manning et al. (2025)](https://www.nature.com/articles/s43247-025-02176-6) investigate antecedent rainfall, wind direction, and season together, while warning about shared weather drivers and limited causal attribution. The exact source/version and claim matrix is frozen in [`PRIMARY_LITERATURE_SNAPSHOT.json`](PRIMARY_LITERATURE_SNAPSHOT.json). Additional work on vegetation and fragility must be checked before making a priority claim.

The *potential* contribution, if the comparisons succeed, is a **small bounded process graph for county customer-stock forecasting that provides testable temporal ordering and geography-conditioned transition responses under auditable public-data transfer tests**. This should initially be framed as a design-and-evaluation contribution, not a foundational architecture or “first” claim. A positive result must beat a Zhu-style discounted-weather/static-vulnerability control and a parameter-matched generic path-state control in addition to W+Cin, causal summaries, and the static fragility mixture. Without a known county-scale governing PDE, adding an arbitrary residual penalty and calling the model a PINN would be misleading. Published [PINN gradient-pathology analysis](https://doi.org/10.1137/20M1318043) also argues against assuming a PDE-penalty architecture is easy to optimize here.
...
Promotion requires a stable margin over W+Cin, the direct/summary, Zhu-style
discounted-weather, fragility, and parameter-matched generic-state controls on held-out tests
