...
def _write_oof(oof, out_dir, yt, m, fips, panel, origin, origin_id, a, cfg):
    ...
    for name, st in oof.items():
        ...
        np.savez_compressed(out_dir / f"oof_{name}.npz", pred=st["pred"], fold_of=st["fold_of"],
                            ...
                            split_unit=str(cfg.get("split_unit", "county-legacy")),
                            outer_split_digest=str(cfg.get("outer_split_digest", "")),
                            outer_split_seed=np.int32(cfg.get("outer_split_seed", -1)),
                            inner_split_seed=np.int32(cfg.get("inner_split_seed", -1)),
                            clock_digest=str(cfg.get("clock_digest", "")),
                            mask_digest=str(cfg.get("mask_digest", "")),
                            metric_digest=str(cfg.get("metric_digest", "")),
                            schema_version=np.int32(cfg.get("schema_version", 1)),
